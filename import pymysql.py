...
def uploadimage():
    try:
        if 'file' not in request.files:
            ...
            return render_template('index.html', msg='No file part in request')
        ...
        if file and allowed_file(file.filename):
            ...
            # Get user ID from session (or update with your logic)
            Id = session.get('id', None)
            if not Id:
                print("⚠️ User ID not found in session")
                return render_template('index.html', msg='User not logged in')
            ...
            return render_template('index.html', msg='Only image files are allowed')

    except Exception as e:
        print("🔥 Unexpected error:", e)
        return render_template('index.html', msg='Something went wrong while uploading.')
